...
RUN chmod a+x /opt/docker/*.sh; \
	...
	ghostscript\
	build-essential; \
	/usr/bin/wget --user-agent="Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) HalloWelt-dfd/3.1.5" -O /opt/docker/pkg/BlueSpice-free.zip https://bluespice.com/filebase/bluespice-free-3-1-5/ --progress=bar --show-progress
